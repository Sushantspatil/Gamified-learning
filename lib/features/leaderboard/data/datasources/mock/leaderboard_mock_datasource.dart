...
class _FictionalPlayer {
  ...
  const _FictionalPlayer({required this.name, required this.avatarId, required this.xp});
}
  ...
  Future<List<LeaderboardEntryModel>> getGlobalLeaderboard({
    ...
    final combined = [
      for (final player in _fictionalPlayers)
        (id: player.name, name: player.name, avatarId: player.avatarId, xp: player.xp, isMe: false),
      (
        id: currentUserId,
