...
void main() {
  ...
  test('wallet debit updates balance in real time', () async {
    ...
    await container.read(walletControllerProvider.notifier).credit(
    ...
    // Debit for power-up
    final didDebit = await container.read(walletControllerProvider.notifier).debit(
      currency: CurrencyType.coins,
      amount: 20,
  ...
  test('QuizController auto-disposes so a topic can be replayed cleanly', () async {
    final storage = await LocalStorageService.create();
    final container = ProviderContainer(
      overrides: [
        localStorageServiceProvider.overrideWithValue(storage),
