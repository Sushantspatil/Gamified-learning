...
void main() {
  group('ChestMockDatasource', () {
    test('daily chest is available once, then blocked until it resets', () async {
    ...
    test('daily and ad chests do not affect each other\'s availability', () async {
  ...
  group('SpinWheelMockDatasource', () {
    test('segments are a fixed, non-empty catalog', () async {
      ...
      expect(segments.map((s) => s.id).toSet().length, segments.length); // unique ids
    });

    test('spin picks a segment id from the catalog and then blocks until reset', () async {
